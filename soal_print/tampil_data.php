...
<div class="row flex-lg-row-reverse align-items-center mb-5">
    ...
    <div class="col-lg-8 col-xl-9">
        <form action="?halaman=pencarian" method="post" class="form-search needs-validation" novalidate>
            <input type="text" name="kata_kunci" class="form-control rounded-pill" placeholder="Cari produk ..." autocomplete="off" required>
            <div class="invalid-feedback">Masukan ID atau Nama produk yang ingin Anda cari.</div>
        </form>
    </div>
</div>
...
if (isset($_GET['pesan'])) {
    ...
    if ($_GET['pesan'] == 1) {
        ...
        echo '<div class="alert alert-success alert-dismissible rounded-4 fade show mb-4" role="alert">
                <strong><i class="fa-solid fa-circle-check me-2"></i>Sukses!</strong> Data produk berhasil disimpan.
    ...
    elseif ($_GET['pesan'] == 2) {
        ...
        echo '<div class="alert alert-success alert-dismissible rounded-4 fade show mb-4" role="alert">
                <strong><i class="fa-solid fa-circle-check me-2"></i>Sukses!</strong> Data produk berhasil diubah.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>';
    }
    // jika pesan = 3
    elseif ($_GET['pesan'] == 3) {
        // tampilkan pesan sukses hapus data
        echo '<div class="alert alert-success alert-dismissible rounded-4 fade show mb-4" role="alert">
                <strong><i class="fa-solid fa-circle-check me-2"></i>Sukses!</strong> Data produk berhasil dihapus.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>';
    }
}
...
<div class="row mb-5">
    ...
    */
    ...
    // sql statement untuk menampilkan data dari tabel "produk"
    // $query = $mysqli->query("SELECT id, name, price, stock FROM produk
    //                          ORDER BY id DESC LIMIT $batas_awal, $batas")
    //                          or die('Ada kesalahan pada query tampil data : ' . $mysqli->error);
    $query = $mysqli->query("SELECT id, name, price, stock, formula FROM produk
                             ORDER BY id DESC LIMIT $batas_awal, $batas")
                             or die('Ada kesalahan pada query tampil data : ' . $mysqli->error);
    ...
    if ($rows <> 0) {
            ...
            <div class="p-2">
                <div class="d-flex bg-white rounded-4 shadow-sm">
                    <!-- <div class="flex-shrink-0 p-3">
                        <img src="images/<?php echo $data['stock']; ?>" class="border border-2 img-fluid rounded-4" alt="Foto Profil" width="100" height="100">
                    </div> -->
                    <div class="p-4 flex-grow-1">
                        <h5><?php echo $data['id']; ?> - <?php echo $data['name']; ?> - <?php echo $data['price']; ?> - <?php echo $data['stock']; ?> - <?php echo $data['formula']; ?></h5>
                        <!-- <p class="text-muted"><?php echo $data['name']; ?></p> -->
                    </div>
                    <div class="p-4">
                        <div class="d-flex flex-column flex-lg-row">
                            <!-- button form detail data Detail-->
                            <a href="?halaman=detail&id=<?php echo $data['id']; ?>" class="btn btn-primary btn-sm rounded-pill px-3 me-2 mb-2 mb-lg-0"> Kalkulator </a>
                            <!-- button form ubah data -->
                            <a href="?halaman=ubah&id=<?php echo $data['id']; ?>" class="btn btn-success btn-sm rounded-pill px-3 me-2 mb-2 mb-lg-0"> Ubah </a>
                            <!-- button modal hapus data -->
                            <button type="button" class="btn btn-danger btn-sm rounded-pill px-3" data-bs-toggle="modal" data-bs-target="#modalHapus<?php echo $data['id']; ?>"> Hapus </button>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            <h1 class="modal-title fs-5" id="exampleModalLabel">
                                <i class="fa-regular fa-trash-can me-2"></i> Hapus Data produk
                            </h1>
                        </div>
                        <div class="modal-body">
                            <p class="mb-2">Anda yakin ingin menghapus data produk?</p>
                            <!-- informasi data yang akan dihapus -->
                            <p class="fw-bold mb-2"><?php echo $data['id']; ?> <span class="fw-normal">-</span> <?php echo $data['price']; ?></p>
                        </div>
        ...
        <div class="d-flex flex-column flex-xl-row align-items-center mt-4">
            ...
            <div class="flex-grow-1 text-center text-xl-start text-muted mb-3">
                <?php
                // sql statement untuk menampilkan jumlah data pada tabel "produk"
                $query = $mysqli->query("SELECT id FROM produk")
                ...
                */
                ...
                // sql statement untuk menampilkan jumlah data pada tabel "produk" yang ditampilkan per halaman
                $query = $mysqli->query("SELECT id FROM produk LIMIT $data_awal, $batas")
